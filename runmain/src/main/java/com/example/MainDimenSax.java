...
public class MainDimenSax {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("基准dp123123：" + baseDP + " dp");
        System.out.println("待适配的屏幕dp参数: " + dataSet.toString());
        //获取当前目录的结对路径
        baseDirPath = new File("runmain/").getAbsolutePath();
        ...
        if (!testBaseDimenFile.exists()) {
            System.out.println("DK WARNING:  \"../values/dimens.xml\" 路径下的文件找不到!");
            return;
        }
        //解析源dimens.xml文件
        list = readBaseDimenFile(baseDimenFilePath);
        if (list == null || list.size() <= 0) {
            System.out.println("DK WARNING:  \"../values/dimens.xml\" 文件无数据!");
            return;
        } else {
            System.out.println("OK \"../values/dimens.xml\" 基准dimens文件解析成功!");
        }
